...
const pizzas = (state = initialState, action: ActionsType): InitialPizzasStateType => {
  switch (action.type) {
    case SET_PIZZAS:
      return { ...state, items: action.payload as Array<PizzaItemType>, isLoaded: true }
    case SET_LOADING:
...
export const fetchPizzas =
  (sortBy: SortByType, category: number | null): ThunkType =>
  async dispatch => {
    dispatch(pizzasActions.setLoading(false))

    const res = await axios.get(
      `https://my-json-server.typicode.com/vlad4k5/react-pizza/pizzas?${
